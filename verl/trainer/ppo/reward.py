...
def get_custom_reward_fn(config: DictConfig, verifier_reward: bool = False) -> Optional[RawRewardFn]:
    ...
    """
    if not verifier_reward:
        reward_fn_config = config.get("custom_reward_function") or {}
        file_path = reward_fn_config.get("path")
    else:
        reward_fn_config = config.get("custom_ver_reward_function") or {}
        file_path = reward_fn_config.get("path")
    if not file_path:
        return None
...
def get_custom_val_reward_fn(config: DictConfig) -> Optional[RawRewardFn]:
    """Load and return a custom reward function from external file.
...
def load_reward_manager(
    config: DictConfig, tokenizer: Any, num_examine: int, verifier_reward: bool = False, **reward_kwargs: Any
    ...
    if not verifier_reward:
        ...
        reward_manager_name = config.reward_model.get("ver_reward_manager", "naive")
        reward_manager_cls = get_reward_manager_cls(reward_manager_name)

    # Try to get a custom reward function based on the configuration
    if num_examine == 0:
        compute_score = get_custom_reward_fn(config, verifier_reward=verifier_reward)
    elif num_examine == 1:
        compute_score = get_custom_val_reward_fn(config)
