...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        self.role = role
        assert self.role in ["actor", "rollout", "ref", "ver_ref", "actor_rollout", "ver_actor_rollout", "actor_rollout_ref"]
        self.is_verifier = self.role in ["ver_ref", "ver_actor_rollout", "ver_actor_rollout_ref"]
        self._is_actor = self.role in ["actor", "actor_rollout", "ver_actor_rollout", "actor_rollout_ref"]
        self._is_rollout = self.role in ["rollout", "actor_rollout", "ver_actor_rollout", "actor_rollout_ref"]
        self._is_ref = self.role in ["ref", "actor_rollout_ref", "ver_ref"]
    ...
    def _build_model_optimizer(
        ...
        # if model_path.split('/')[-1][:7] == 'Qwen2.5':
        #     from verl.mod_transformers.configuration_auto import AutoConfig
        #     from verl.mod_transformers.modeling_auto import AutoModelForCausalLM, AutoModelForVision2Seq
        # else:
        #     from transformers import AutoConfig, AutoModelForCausalLM, AutoModelForVision2Seq
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if self.is_verifier:
                temp_dict = {}
                metric_prefix = 'ver_actor'
                for k, v in metrics.items():
                    temp_dict.update({f"{metric_prefix}_{k}": v})
                del metrics
                metrics = temp_dict
            delta_time = timer.last
            global_num_tokens = data.meta_info["global_token_num"]
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics[f"perf/mfu/{metric_prefix}"] = (
                estimated_flops * self.config.actor.ppo_epochs / promised_flops / self.world_size
            )
            metrics[f"perf/{metric_prefix}_max_memory_allocated_gb"] = get_torch_device().max_memory_allocated() / (1024**3)
            metrics[f"perf/{metric_prefix}_max_memory_reserved_gb"] = get_torch_device().max_memory_reserved() / (1024**3)
            metrics[f"perf/{metric_prefix}_cpu_memory_used_gb"] = psutil.virtual_memory().used / (1024**3)

            lr = self.actor_lr_scheduler.get_last_lr()[0]
            metrics[f"{metric_prefix}/lr"] = lr
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        timing_generate = {}
        # log_gpu_memory_usage('Before rollout sharding manager...', logger=None)
        ...
        log_gpu_memory_usage('After leaving rollout sharding manager...', logger=logger)
        ...
        log_gpu_memory_usage('After clearing kv cache', logger=logger)
        return output
    
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="red", role="together_rollout_generate")
    def generate_sequences_wosharding(self, prompts: DataProto):
        # Support all hardwares
        prompts = prompts.to(get_device_id())
        ...
        timing_generate = {}
        # log_gpu_memory_usage('Before rollout sharding manager...', logger=None)
        log_gpu_memory_usage("After entering rollout sharding manager", logger=None)
        log_gpu_memory_usage("Before rollout generation", logger=None)
        with simple_timer("generate_sequences", timing_generate):
            output = self.rollout.generate_sequences(prompts=prompts)
        log_gpu_memory_usage("After rollout generation", logger=None)
        # log_gpu_memory_usage('After leaving rollout sharding manager...', logger=None)
        timing_generate.update(self.rollout_sharding_manager.timing)
        # We calculate the average timing across all ranks
        # to make sure meta_info["timing"] is the same
        timing_generate = reduce_timing(timing_generate)
        ...
        # clear kv cache
        # get_torch_device().empty_cache()
        # log_gpu_memory_usage('After clearing kv cache...', logger=None)
        return output
    
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="red", role="starting_sharding_manager")
    def start_sharding_manager(self):
        # Support all hardwares
        self.rollout_sharding_manager.start()
    
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="red", role="exiting_sharding_manager")
